<nav class="bg-white border-b-2 border-gray-200" x-data="{ open: false }">
    <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div class="flex h-14 items-center justify-between">
            <div class="flex items-center">
                <div class="shrink-0">
                    <a href="/">
                        <img src="{{ asset('img/logo.png') }}" alt="Your Company" class="size-36" />
                    </a>
                </div>
                <div class="hidden md:block">
                    <div class="ml-10 flex items-baseline space-x-4">
                        ...
                        @else
                            ...
                            <x-nav-link href="/about" :active="request()->is('about')">Series</x-nav-link>
                            <x-nav-link href="/contact" :active="request()->is('contact')">Topics</x-nav-link>
                        @endif
                    </div>
                </div>
            </div>
            <div class="hidden md:block">
                <div class="ml-4 flex items-center md:ml-6">
                    ...
                    @guest
                        <x-button data-modal-target="login-modal" data-modal-toggle="login-modal" variant="outline"
                            class="mr-3">Sign in</x-button>
                        <x-button data-modal-target="register-modal" data-modal-toggle="register-modal"
                    ...
                    @auth
                        <div class="relative ml-3">
                            <button @click="open = !open"
                                class="relative flex max-w-xs items-center rounded-full cursor-pointer">
                            ...
                            <div x-show="open" @click.outside="open = false" x-transition
                                ...
                                <form action="{{ route('logout') }}" method="POST">
                                    @csrf
                                     <button type="submit" class="px-4 py-2 text-sm font-medium text-black">
                                        Logout
                                    </button>
                                </form>
                            </div>
            ...
            <div class="-mr-2 flex md:hidden">
                <!-- Mobile menu button -->
                <button type="button" @click="open= !open" command="--toggle" commandfor="mobile-menu"
                    class="relative inline-flex items-center justify-center rounded-md p-2
        text-gray-400 hover:bg-white/5 hover:text-white focus:outline-2 focus:outline-offset-2 focus:outline-indigo-500">
    ...
    <div x-show="open" x-transition class="md:hidden">
        <div class="space-y-1 px-2 pt-2 pb-3 sm:px-3">
            ...
            <x-nav-link href="/about" :active="request()->is('about')">Post</x-nav-link>
            <x-nav-link href="/contact" :active="request()->is('contact')">Post</x-nav-link>
        </div>
